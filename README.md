...
```bash
# Clone le dépôt
git clone https://github.com/Elhadj098/KAYOR_XPLR.git
cd KAYOR_XPLR
...
│
├── scripts/
│   ├── extract_data.py               # Extraction .mcap → .csv
│   ├── synchronize_data.py           # Synchronisation temporelle
│   ├── analyze_metrics.py            # Calcul des métriques
│   └── plot_analysis.py              # Génération des graphiques
│
├── data/
│   ├── raw/                          # Données brutes (rosbag2)
│   ├── processed/                    # Données synchronisées
│   └── results/                      # Résultats d'analyse
│
├── results/
│   ├── Rapport_Analyse.pdf           # Rapport d'ingénierie complet
│   ├── Graphique_Distinct_X_Y.png    # Comparaison axes X/Y
│   └── metrics_summary.json          # Résumé des métriques
│
└── docs/
...
- **`Rapport_Analyse.pdf`** : Rapport d'ingénierie complet détaillant les calculs, le développement de la pseudo-inverse de Moore-Penrose pour le modèle cinématique, et l'interprétation des phénomènes observés.

- **`Graphique_Distinct_X_Y.png`** : Visualisation montrant la superposition parfaite sur l'axe X et le décrochage du patinage sur l'axe Y.

- **`metrics_summary.json`** : Export structuré de toutes les métriques calculées en format exploitable.

### 🐍 Scripts Python

| Script | Description |
|--------|-------------|
| `extract_data.py` | Convertit rosbag2 (.mcap) en fichiers CSV standardisés |
| `synchronize_data.py` | Réaligne tous les topics sur une grille temporelle uniforme |
| `analyze_metrics.py` | Calcule latence, patinage, et caractéristiques dynamiques |
| `plot_analysis.py` | Génère visualisations matplotlib |
...
# 2. Source l'environnement ROS 2
source /opt/ros/jazzy/setup.bash
echo "source /opt/ros/jazzy/setup.bash" >> ~/.bashrc

# 3. Clone et configure le projet
git clone https://github.com/Elhadj098/KAYOR_XPLR.git
cd KAYOR_XPLR
...
# 6. Lance les tests
python scripts/analyze_metrics.py --test
```
...
Ce protocole et les données associées servent de base de connaissances pour entraîner et valider un algorithme correcteur par Intelligence Artificielle (IA), dont le but sera de lisser les commandes de vitesse en compensant les phénomènes identifiés.

**Objectifs futurs :**
- 🤖 Développer un modèle prédictif de latence/patinage
- 🧠 Entraîner un réseau de neurones pour correction en temps réel
- 📊 Valider les résultats sur le robot physique
- 📈 Extrapoler à d'autres configurations de terrain
...
- Support de multiples formats de rosbag (DB3, SQLite)
...
- 🤖 [Documentation ROS 2 Jazzy](https://docs.ros.org/en/jazzy/)
- 🛞 [Théorie des roues Mecanum](https://en.wikipedia.org/wiki/Mecanum_wheel)
- 📚 [MentorPi M1 Architecture](https://www.mentoroboticslab.eu/)
- 📖 [Modélisation Cinématique](https://en.wikipedia.org/wiki/Kinematics)
